...
# add third parameter as training corpus size
size = int(sys.argv[3])
# size = 5000
# print("size is " + sys.argv[3])

indexSet = set(random.sample(range(0, 39832), size))
print(len(indexSet))
...
for prevtag in transitions:
    for tag in transitions[prevtag]:
        print "trans %s %s %s" % (prevtag, tag, float(
