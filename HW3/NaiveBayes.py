...
class NaiveBayes:
    ...
    def __init__(self):
        ...
        self.totalDocs = 0.0
        self.totalPosDocs = 0.0
        self.totalNegDocs = 0.0
        self.totalPosWords = 0.0
        self.totalNegWords = 0.0
        self.vocab = set()

        self.posWords = collections.defaultdict(lambda: 0)
        self.negWords = collections.defaultdict(lambda: 0)
    ...
    def classify(self, words):
        ...
        """
        # With stop list, the accuracy is 0.811000
        if self.stopWordsFilter:
            words = self.filterStopWords(words)

        # add-one smoothing for normal NB
        adding = 1

        # The accuracy is 0.829500 with binary NB
        if self.naiveBayesBool:
            words = set(words)
            adding = 0.6

        # I used negation combined with binary NB as my bestModel
        # The accuracy is 0.836000
        if self.bestModel:
            words = self.negation(words)
            words = set(words)
            adding = 0.6

        posProb = math.log(self.totalPosDocs/self.totalDocs)
        negProb = math.log(self.totalNegDocs/self.totalDocs)

        for word in words:
            if word in self.posWords:
                posProb += math.log((self.posWords[word] + adding) /
                                    (self.totalPosWords+len(self.vocab)))
            else:
                posProb += math.log(adding /
                                    (self.totalPosWords+len(self.vocab)))

            if word in self.negWords:
                negProb += math.log((self.negWords[word] + adding) /
                                    (self.totalNegWords+len(self.vocab)))
            else:
                negProb += math.log(adding /
                                    (self.totalNegWords+len(self.vocab)))
    ...
    # a function that add NOT_ to every word between negation and following punctuation
    # it will return the modified words list
    def negation(self, words):
        negationWords = {"not", "never", "barely", "no",
                         "nothing", "Not", "Never", "Barely", "No", "Nothing"}
        negation = "n't"
        NOT_ = "NOT_"
        negFlag = False
        modifiedWords = []
        ...
        for word in words:
            if negFlag:
                # if there's punctuation, stop the negation
                for char in word:
                    if char in string.punctuation:
                        negFlag = False
                        break

            if negation in word or word in negationWords:
                negFlag = True
    ...
    def buildSplits(self, args):
        ...
        trainDir = args[0]
        if len(args) == 1:
            print '[INFO]\tOn %d-fold of CV with \t%s' % (
                self.numFolds, trainDir)
...
def main():
    ...
    if len(args) == 2 and os.path.isfile(args[1]):
        classifyFile(stopWordsFilter, naiveBayesBool,
                     bestModel, args[0], args[1])
    else:
